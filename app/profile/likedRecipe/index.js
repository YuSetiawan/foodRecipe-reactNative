...
import {Icon, Box, ScrollView, NativeBaseProvider, Center, Image, Text, View, Divider, HStack, Button} from 'native-base';
import {Link, useRouter} from 'expo-router';
import FooterMenu from '../../../components/footerTabs';
import {useDispatch} from 'react-redux';
import AsyncStorage from '@react-native-async-storage/async-storage';
import {useState, useEffect} from 'react';
import {deleteLikeActions} from '../../config/redux/actions/likeAction';
...
const MyRecipe = () => {
  ...
  const getData = async () => {
    ...
    console.log(idUser);
    axios
      .get(`https://food-recipe-server-six.vercel.app/likeds/${idUser}`)
      .then((response) => {
            ...
            <>
              <View style={styles.boxListRecipe}>
                ...
                <View w={40}>
                  ...
                  <Text style={styles.textListB}>Recipe by {recipe.name}</Text>
                </View>
                <Box margin="8">
                  <Icon size="7" color="#D2122E" as={<FeatherIcon name="thumbs-up" />} onPress={() => setModalVisible(!modalVisible)} />
                  {/* <Icon size="7" color="#D2122E" as={<FeatherIcon name="thumbs-up" />} onPress={() => handleDelete(recipe.likeds_id)} /> */}
                  <Text color="#D2122E" marginLeft={-1}>
                    Unlike
                  </Text>
                  <Modal
                    animationType="slide"
                  ...
                  >
                    <View style={styles.modalView}>
                      <Text marginLeft={'auto'} marginRight={'auto'} fontSize={20}>
                        Sure want to unlike this recipe?
                      </Text>
                      ...
                      <HStack mt={5} style={{marginLeft: 'auto', marginRight: 'auto'}}>
                        ...
                        <Button backgroundColor="#D2122E" w={20} onPress={() => handleDelete(recipe.likeds_id)}>
                          Delete
                        </Button>
                      </HStack>
                    </View>
